...
var entryArr = [fastFood, groceries, cinema];
...
function loadEntriesToContentArea() {
    // <li onclick="loadEntry()">
    ...
    for(i=0; i<5; i++){
        entryArr.forEach(function(value, index){
            var entry = $('<li><i></i><div></div><div></div></li>');
            entry.on('click', function() {
                loadEntry();
            });
            entry.find('i').html(value.category).addClass('material-icons');
            entry.find('div:first').addClass('entry-content')
                                   .append($('<span>').html(value.label))
                                   .append($('<span>').html(value.date.toLocaleDateString()));
            entry.find('div:last').html(value.amount + ' €');
            $('#content-area ul').append(entry);            
        })
    }
}
